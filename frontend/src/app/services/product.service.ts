...
export class ProductService {
    ...
    getAllInPage(page: number, size: number , name : string): Observable<any> {
        const url = `${this.productUrl}?page=${page}&size=${size}&name=${name}`;
    ...
    getAllInPages(page: number, size: number ): Observable<any> {
        const url = `${this.productUrl}s?page=${page}&size=${size}`;
        return this.http.get(url)
            .pipe(
                // tap(_ => console.log(_)),
            )
